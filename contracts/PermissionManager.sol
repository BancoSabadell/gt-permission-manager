...
contract PermissionManager {
  ...
  function isNetworkAdmin(address who) {
    if(!_networkAdmin[_rol[who]])
      throw;
  }

  function isIdentityManager(address who) {
    if(!_networkAdmin[_rol[who]] && !_identityManager[_rol[who]])
      throw;
  }

  function isReputationProvider(address who) {
    if(!_networkAdmin[_rol[who]] && !_reputationProvider[_rol[who]])
      throw;
  }

  function isInteractionAllowed(address from, address to, uint interaction) {
    if(!_relationships[_rol[from]][_rol[to]][interaction])
      throw;
  }
